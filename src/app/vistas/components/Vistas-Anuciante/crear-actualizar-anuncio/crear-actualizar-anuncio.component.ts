...
export class CrearActualizarAnuncioComponent implements OnInit {
 ...
 logLoadTime() {
  window.addEventListener('load', () => {
    ...
    if (navEntry) {
      console.log('⏱️ Tiempo total de carga en crear actualizar anuncio (domComplete):', navEntry.domComplete.toFixed(2), 'ms');
      console.log('🧱 Tiempo de render en crear actualizar anuncio (domContentLoaded):', navEntry.domContentLoadedEventEnd.toFixed(2), 'ms');
      console.log('🌐 Tiempo de respuesta crear actualizar anuncio (responseEnd):', navEntry.responseEnd.toFixed(2), 'ms');
    } else {
