...
} from "./chunk-U42CI6GS.js";
import {
  BidiModule,
  Directionality
} from "./chunk-QBQSPOVY.js";
import {
  DOCUMENT,
  Location
...
var MatDialogRef = class {
  ...
  _beforeClosed = new import_rxjs8.Subject();
  ...
  constructor(_ref, config, _containerInstance) {
    ...
    _containerInstance._animationStateChanged.pipe((0, import_operators6.filter)((event) => event.state === "opened"), (0, import_operators6.take)(1)).subscribe(() => {
      this._afterOpened.next();
      this._afterOpened.complete();
    });
    _containerInstance._animationStateChanged.pipe((0, import_operators6.filter)((event) => event.state === "closed"), (0, import_operators6.take)(1)).subscribe(() => {
    ...
    (0, import_rxjs8.merge)(this.backdropClick(), this.keydownEvents().pipe((0, import_operators6.filter)((event) => event.keyCode === ESCAPE && !this.disableClose && !hasModifierKey(event)))).subscribe((event) => {
      if (!this.disableClose) {
        event.preventDefault();
  ...
  close(dialogResult) {
    ...
    this._containerInstance._animationStateChanged.pipe((0, import_operators6.filter)((event) => event.state === "closing"), (0, import_operators6.take)(1)).subscribe((event) => {
